...
현재 1차 구현은 Place 등록, 승인/반려, 승인된 장소 목록/지도 조회까지만 다룹니다. 리뷰, 신고, 숨김/복구는 후속 MVP에서 구현합니다.
...
- 장소 상세 조회가 가능하다.
- `PENDING`, `REJECTED` 장소는 일반 목록과 지도 마커 API에 포함되지 않는다.
- 주요 Service 로직에 테스트 코드가 존재한다.
- README 또는 docs에 실행 방법, 주요 기능, API 요약이 정리되어 있다.
